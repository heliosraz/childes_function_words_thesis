...
counts_chi_tokens$n<-round(((counts_chi_tokens$n)/sum(counts_chi_tokens$n)),digits=5)
...
counts_parent_tokens<-count(parent_english_tokens,word)%>%filter(word!="other")
counts_parent_tokens$n<-round(((counts_parent_tokens$n)/length(parent_english_tokens$word)),digits=5)
...
combined_totals<-rbind(counts_chi_tokens,counts_parent_tokens)

ggplot(data=combined_totals, aes(x=reorder(word,n), y=n, fill=speaker_role)) +
  geom_bar(stat="identity", position="dodge",width=0.4)+
  geom_text(aes(label=n), hjust=-0.3, size=2)+
  theme_minimal()+labs(title="Token Frequencies", 
         x="Frequencies", y = "Tokens")+ coord_flip()
